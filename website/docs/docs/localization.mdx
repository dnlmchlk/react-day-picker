...
DayPicker supports the Jalali (Persian) calendar through the [date-fns-jalali](https://www.npmjs.com/package/date-fns-jalali) package.

To use the Jalali calendar, import `DayPicker` from `react-day-picker/jalali`:
...
By defaults, Jalali DayPicker uses the Persian/Farsi locale (Iran) and a `dir` from right to left. This calendar returns regular dates. Use the exported `jalaliDateLib` object to format dates to Jalali.
